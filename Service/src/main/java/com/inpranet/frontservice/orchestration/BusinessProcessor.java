...
public class BusinessProcessor implements IBusinessProcessor {
	...
	/** Logger */
	private static Logger log = Logger.getLogger(BusinessProcessor.class.getName());
	...
	public void receiveRawPositions(final User user, final GeoPos pos) {
		...
		List<Zone> zones = zoneService.getZoneListFromPos(pos.getLongitude(), pos.getLatitude());
	...
	/**
	 * Processus metuer d'authentification de l'utilisateur via un token (session)
	 * @param pos
	 */
	...
	public Collection<Document> receiveDocumentOrder(User user) {
		...
		List<Zone> zones = habitService.deduceZone(user, planningHorizon, interestList);
