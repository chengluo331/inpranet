...
public class TemporalRegexEngine extends RegexEngine {
	...
	static {
		...
		loadRegexLists("C:\\Temp\\temporalRegexLists.txt");
	}
	 ...
	 */
	public static TemporalRegexResults TemporalRegexSearch(String text) {
		Pattern pattern;
		Matcher matcher;
		Date date;
		...
		for (int i = 0; i < patterns.size(); i++) {
			...
			while (matcher.find()) {
				...
				try {
					simpleDateFormat = new SimpleDateFormat(formatMappers.get(i));
