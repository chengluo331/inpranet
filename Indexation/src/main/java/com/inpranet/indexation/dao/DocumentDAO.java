...
public class DocumentDAO implements IDocumentDAO {
	...
	public void createDocument(Document document) {
		...
		final String INSERT_DOCUMENT = "INSERT INTO indexation.document (id, title, uri, start_date, end_date, latitude, longitude, data) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
		ApplicationContext applicationContext = new ClassPathXmlApplicationContext("indexation-data.xml");
		JdbcTemplate jdbcTemplate = (JdbcTemplate) applicationContext.getBean("jdbcTemplate");
		
		// Execution de la requete
		jdbcTemplate.update(INSERT_DOCUMENT, new Object[] {document.getId(), document.getTitle(), document.getUri(), document.getStart_date(), document.getEnd_date(), document.getLatitude(), document.getLongitude(), document.getData()});
	}
	...
	public class DocumentRowMapper implements RowMapper<Document> {
		...
		public Document mapRow(ResultSet rs, int rowNum) throws SQLException {
			Document document = new Document(rs.getInt("id"), rs.getString("title"), rs.getString("uri"), rs.getDate("start_date"), rs.getDate("end_date"), rs.getFloat("latitude"), rs.getFloat("longitude"), rs.getString("data"));
			return document;
		}
	}
